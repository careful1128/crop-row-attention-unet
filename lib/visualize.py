...
def concat_result(ori_img,pred_res,gt):
    ...
    binary[binary>=0.5]=1
    binary[binary<0.5]=0  
    # pred_res = pred_res / np.max(pred_res)
    ...
    if ori_img.shape[2]==1:
        ...
        binary = np.repeat((binary*255).astype(np.uint8),repeats=3,axis=2)
        gt = np.repeat((gt*255).astype(np.uint8),repeats=3,axis=2)
        ori_img = np.repeat((ori_img*255).astype(np.uint8),repeats=3,axis=2)

    colormap = plt.get_cmap('inferno')
    pred_res = (colormap(pred_res[:, :, 0]) * 2**16).astype(np.uint16)[:,:,:3] / (2**16) * 255
