...
该部分主要包含图像块(image patches)提取的相关功能。
在训练阶段,在fov(可选)中随机提取图像块(image patches),
并且测试阶段需要在分裂后进行拼接
"""
import numpy as np
import random
...
def load_file_path_txt(file_path):
    img_list = glob.glob(f'{file_path}/imgs/*')
    gt_list = glob.glob(f'{file_path}/masks/*')
    fov_list = glob.glob(f'{file_path}/fovs/*')

    # import random
    # random.seed(0)
    # img_list = random.sample(img_list, 200)
    # random.seed(0)
    # gt_list = random.sample(gt_list, 200)
    # random.seed(0)
    # fov_list = random.sample(fov_list, 200)
...
# 按顺序加载数据集的原始图像、groundtruth和FOV，并检查尺寸
...
#加载原始数据，返回the extracted patches 进行训练
...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    ...
    # for i in range(final_avg.shape[0]):
    #     o = final_avg[i, 0]
    #     r = cv2.blur(o, (3,3))
    #     final_avg[i, 0] = r
        

    # patchSize = np.array(preds.shape[2:])
    # patchSize = np.array([27, 27])
    # print(patchSize)
    # patchSride = patchSize // 2
    # result = np.zeros(final_avg.shape)
    # normalization = np.zeros(final_avg.shape)
    # gaussian_map = get_gaussian(patchSize)[np.newaxis, np.newaxis, ...]
    # for i in range(0, final_avg.shape[2]-patchSize[0]+1, patchSride[0]):
    #     for j in range(0, final_avg.shape[3] - patchSize[1] + 1, patchSride[1]):
    #         patch = final_avg[:, :, i:i+patchSize[0], j:j+patchSize[1]].astype(np.float32)
    #         patch *= gaussian_map
    #         normalization[:, :, i:i+patchSize[0], j:j+patchSize[1]] += gaussian_map
    #         result[:, :, i:i+patchSize[0], j:j+patchSize[1]] += patch
    # final_avg = result / normalization
    # final_avg = np.nan_to_num(final_avg)
