...
def create_patch_idx(img_fovs, args):
    ...
    while count < args.N_patches:
        ...
        #         continue
        if np.sum(img_fovs[n,0][y_center-int(args.train_patch_height/2):y_center+int(args.train_patch_height/2),x_center-int(args.train_patch_width/2):x_center+int(args.train_patch_width/2)]) <= 5 :
            continue
