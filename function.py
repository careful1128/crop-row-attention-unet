...
def get_dataloaderV2(args):
    ...
    """
    global train_set
    imgs_train, masks_train, fovs_train = data_preprocess(data_path_list = args.data_path)
    # print(np.sum(masks_train), masks_train.reshape(-1).shape, np.sum(masks_train)/masks_train.reshape(-1).shape)

    patches_idx = create_patch_idx(fovs_train, args)
    # patches_idx = create_patch_idx(masks_train, args)

    train_idx,val_idx = np.vsplit(patches_idx, (int(np.floor((1-args.val/100)*patches_idx.shape[0])),))
    # new_train =  np.empty((patches_idx.shape[0],64, 64),dtype=int)
